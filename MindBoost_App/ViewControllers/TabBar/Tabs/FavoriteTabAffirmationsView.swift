//
//  FavoriteTabAffirmationsView.swift
//  MindBoost_App
//
...
struct FavoriteTabAffirmationsView: View {
    // MARK: - Properties
    @ObservedObject var coordinator: TabBarCoordinator
    var affirmations: [AffirmationAppModel]
    // MARK: - Body
    var body: some View {
        let user = coordinator.viewModel.user
        TabView {
            ForEach(coordinator.viewModel.user.favoriteAffirmations, id: \.id) { affirmation in
                VStack {
                    Spacer()
                    VStack {
        ...
        .background(
            Image(user.settings.background?.backgroundType.imageName ?? "white")
                   .resizable()
                   .scaledToFill()
